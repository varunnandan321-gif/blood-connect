...
export default function LandingPage() {
  ...
  return (
    <div className="bg-[#fdfaf9] min-h-screen flex items-center justify-center overflow-hidden relative font-sans">
      {/* Background Elements */}
      <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none">
        {/* Decorative soft blob behind card */}
        <div className="absolute right-[-10%] bottom-[-20%] w-[800px] h-[800px] bg-rose-50 rounded-full blur-[120px] -z-10"></div>
      ...
      <main className="container mx-auto px-6 lg:px-12 grid grid-cols-1 lg:grid-cols-[1fr_440px] gap-12 lg:gap-24 items-center relative z-10 max-w-7xl">
        ...
        <div className="flex flex-col gap-6 lg:pr-8 pt-10 lg:pt-0">
          {/* Logo Pill */}
          <div className="fade-up-v2 inline-flex items-center gap-3 bg-white px-5 py-2.5 rounded-full border border-gray-100 shadow-sm w-fit mt-12 lg:mt-0">
            ...
            <span className="text-[12px] font-bold text-slate-800 uppercase tracking-[0.15em] leading-none text-center pt-[1px]">Blood Connect</span>
          </div>

          <div className="space-y-4 lg:space-y-6 mt-4">
            <h1 className="text-6xl md:text-7xl xl:text-[6.5rem] font-black text-[#111827] leading-[0.95] tracking-tight">
        ...
        <div className="fade-up-v2 w-full mt-8 lg:mt-0" style={{ animationDelay: '0.4s' }}>
          <div className="bg-white w-full p-8 sm:p-10 rounded-[2.5rem] shadow-[0_20px_50px_-15px_rgba(225,29,72,0.1)]_20px_50px_-15px_rgba(0,0,0,0.5)] transition-all duration-500 relative z-20">
            <div className="mb-6">
              <h2 className="text-[28px] font-bold text-slate-900 tracking-tight">{isLogin ? "Welcome Back" : "Create Account"}</h2>
              <p className="text-slate-500 text-[13px] mt-1.5 font-medium">{isLogin ? "Please sign in to your donor account" : "Join the donor community today"}</p>
            </div>
            ...
            <form className="space-y-5" onSubmit={handleAuth}>
              {!isLogin && (
                <div className="space-y-2.5">
                  ...
                  <div className="relative">
                    ...
                    <input autoFocus required value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="w-full pl-11 pr-4 py-3.5 bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-medical-red/20 focus:border-medical-red outline-none transition-all duration-300 text-slate-900 placeholder:text-slate-400 text-sm" placeholder="John Doe" type="text" />
                  </div>
                </div>
              )}
              ...
              <div className="space-y-2.5">
                ...
                <div className="relative">
                  ...
                  <input required value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} className="w-full pl-11 pr-4 py-3.5 bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-medical-red/20 focus:border-medical-red outline-none transition-all duration-300 text-slate-900 placeholder:text-slate-400 text-sm" placeholder="name@company.com" type="email" />
                </div>
              </div>

              {!isLogin && (
                <div className="space-y-2.5">
                  <label className="text-[13px] font-bold text-slate-700 ml-1">Your Blood Group</label>
                  <div className="relative">
                    <Droplet className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 w-4 h-4 transition-colors" />
                    <select required value={formData.bloodGroup} onChange={(e) => setFormData({ ...formData, bloodGroup: e.target.value })} className="appearance-none w-full pl-11 pr-4 py-3.5 bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-medical-red/20 focus:border-medical-red outline-none transition-all duration-300 text-slate-900 text-sm">
              ...
              <div className="space-y-2.5">
                <div className="flex justify-between items-center px-1">
                  ...
                  {isLogin && <button type="button" onClick={handleForgotPassword} className="text-[11px] font-bold text-medical-red hover:text-red-700:text-red-400 transition-colors">Forgot?</button>}
                </div>
                <div className="relative">
                  ...
                  <input required value={formData.password} onChange={(e) => setFormData({ ...formData, password: e.target.value })} className="w-full pl-11 pr-11 py-3.5 bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-medical-red/20 focus:border-medical-red outline-none transition-all duration-300 text-slate-900 placeholder:text-slate-400 text-sm tracking-widest" placeholder="••••••••" type="password" />
                  <button type="button" className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-300 hover:text-slate-500:text-slate-300 transition-colors">
